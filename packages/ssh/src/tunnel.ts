...
// A cold archive launch also downloads and unpacks a release archive and may
// wait on another installer's lock. The SSH command timeout must outlast the
// archive's own bounded download plus readiness checks.
...
export interface RemoteT3RunnerOptions {
  /** Dev mode: run `node <path>` on the remote source checkout. */
  readonly nodeScriptPath?: string | null;
  readonly nodeEngineRange?: string | null;
  /** Exact release version whose self-contained archive runs remotely. */
...
// it must be one exact SemVer segment: no separators, no `..`, and no shell
