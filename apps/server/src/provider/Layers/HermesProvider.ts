...
const runHermesVersionCommand = (
  ...
  Effect.gen(function* () {
    ...
    const spawnCommand = yield* resolveSpawnCommand(command, ["acp", "--version"], {
      env: environment,
    });
  ...
  if (Exit.isFailure(discoveryExit)) {
    ...
    return buildServerProvider({
      ...
      probe: {
        ...
        auth: { status: "unknown" },
        message:
          "Hermes ACP could not start. Configure Hermes with `hermes acp --setup` and check server logs.",
      },
    });
  }
