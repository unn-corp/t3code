...
describe("findEsmImportsOfExternalPackages", () => {
  ...
  it("allows optional dynamic Effect Bun modules but rejects static imports", () => {
    assert.deepStrictEqual(
      findEsmImportsOfExternalPackages(
        'const load = () => import("@effect/platform-bun/BunHttpServer");',
      ),
      [],
    );
    assert.deepStrictEqual(
      findEsmImportsOfExternalPackages('import { layer } from "@effect/platform-bun/BunServices";'),
